...
function(vscode)
    ...
    set(oneValueKeywords
        ...
        "C_CPP_PROPERTIES_ENABLE_CONFIG_SQUIGGLES"
        "C_CPP_PROPERTIES_CONFIG_NAME"
        "C_CPP_PROPERTIES_CONFIG_COMPILER_PATH"
        "C_CPP_PROPERTIES_CONFIG_INTELLI_SENSE_MODE"
        "C_CPP_PROPERTIES_CONFIG_C_STANDARD"
        "C_CPP_PROPERTIES_CONFIG_CPP_STANDARD"
        "C_CPP_PROPERTIES_CONFIG_PROVIDER"
        "C_CPP_PROPERTIES_CONFIG_WINDOWS_SDK_VERSION"
        "C_CPP_PROPERTIES_CONFIG_COMPILE_COMMANDS"
        "C_CPP_PROPERTIES_CONFIG_DOTCONFIG"
        "C_CPP_PROPERTIES_CONFIG_MERGE_CONFIGS"
        "C_CPP_PROPERTIES_CONFIG_BROWSE_LIMIT_SYMBOLS_TO_INCLUDED_HEADERS"
        "C_CPP_PROPERTIES_CONFIG_BROWSE_DATABASE_FILE"
        "LAUNCH_VAR"
        "LAUNCH_FILE"
        "LAUNCH_INPUT"
        "LAUNCH_VERSION"
    ...
    set(multiValueKeywords
        ...
        "C_CPP_PROPERTIES_CONFIG_INCLUDE_PATH"
        "C_CPP_PROPERTIES_CONFIG_DEFINES"
        ...
        "LAUNCH_CONFIG_PLUGINS"
        "LAUNCH_CONFIG_SETUP_MACROS"
        "LAUNCH_CONFIG_SOURCE_FILE_MAP"
    ...
    if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_VAR}" STREQUAL "" OR NOT "${${currentFunctionName}_C_CPP_PROPERTIES_FILE}" STREQUAL "")
        ...
        if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_ENABLE_CONFIG_SQUIGGLES}" STREQUAL "")
            set(cCppPropertiesEnableConfigSquiggles "${${currentFunctionName}_C_CPP_PROPERTIES_ENABLE_CONFIG_SQUIGGLES}")
        endif()
        ...
        if(NOT "${cCppPropertiesEnableConfigSquiggles}" STREQUAL "")
        ...
        if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_NAME}" STREQUAL "")
            ...
            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_COMPILER_PATH}" STREQUAL "")
                message(FATAL_ERROR "'C_CPP_PROPERTIES_CONFIG_COMPILER_PATH' is empty!")
            else()
                set(cCppPropertiesConfigCompilerPath "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_COMPILER_PATH}")
                cmake_path(APPEND cCppPropertiesConfigCompilerPath "DIR")
                cmake_path(GET "cCppPropertiesConfigCompilerPath" PARENT_PATH cCppPropertiesConfigCompilerPath)
            endif()

            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_COMPILER_ARGS}" STREQUAL "")
                set(cCppPropertiesConfigCompilerArgs "")
            else()
                set(cCppPropertiesConfigCompilerArgs "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_COMPILER_ARGS}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_INTELLI_SENSE_MODE}" STREQUAL "")
                set(cCppPropertiesConfigIntelliSenseMode "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_INTELLI_SENSE_MODE}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_C_STANDARD}" STREQUAL "")
                set(cCppPropertiesConfigCStandard "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_C_STANDARD}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_CPP_STANDARD}" STREQUAL "")
                set(cCppPropertiesConfigCppStandard "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_CPP_STANDARD}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_PROVIDER}" STREQUAL "")
                set(cCppPropertiesConfigProvider "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_PROVIDER}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_WINDOWS_SDK_VERSION}" STREQUAL "")
                set(cCppPropertiesConfigWindowsSdkVersion "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_WINDOWS_SDK_VERSION}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_COMPILE_COMMANDS}" STREQUAL "")
                set(cCppPropertiesConfigCompileCommands "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_COMPILE_COMMANDS}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_DOTCONFIG}" STREQUAL "")
                set(cCppPropertiesConfigDotconfig "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_DOTCONFIG}")
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_MERGE_CONFIGS}" STREQUAL "")
                set(cCppPropertiesConfigMergeConfigs "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_MERGE_CONFIGS}")
            endif()

            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_BROWSE_LIMIT_SYMBOLS_TO_INCLUDED_HEADERS}" STREQUAL "")
                set(cCppPropertiesConfigBrowseLimitSymbolsToIncludedHeaders "true")
            else()
                if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_BROWSE_LIMIT_SYMBOLS_TO_INCLUDED_HEADERS}")
                    set(cCppPropertiesConfigBrowseLimitSymbolsToIncludedHeaders "true")
                else()
                    set(cCppPropertiesConfigBrowseLimitSymbolsToIncludedHeaders "false")
                endif()
            endif()

            if(NOT "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_BROWSE_DATABASE_FILE}" STREQUAL "")
                set(cCppPropertiesConfigBrowseDatabaseFile "${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_BROWSE_DATABASE_FILE}")
            endif()

            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_INCLUDE_PATH}" STREQUAL "")
                set(cCppPropertiesConfigIncludePath "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_INCLUDE_PATH")
                    cmake_path(APPEND v "DIR")
                    cmake_path(GET "v" PARENT_PATH v)
                    if(NOT "${v}" IN_LIST "cCppPropertiesConfigIncludePath")
                        list(APPEND cCppPropertiesConfigIncludePath "${v}")
                    endif()
                endforeach()
            endif()

            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_DEFINES}" STREQUAL "")
                set(cCppPropertiesConfigDefines "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_DEFINES")
                    cmake_path(APPEND v "DIR")
                    cmake_path(GET "v" PARENT_PATH v)
                    if(NOT "${v}" IN_LIST "cCppPropertiesConfigDefines")
                        list(APPEND cCppPropertiesConfigDefines "${v}")
                    endif()
                endforeach()
            endif()

            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_MAC_FRAMEWORK_PATH}" STREQUAL "")
                set(cCppPropertiesConfigMacFrameworkPath "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_MAC_FRAMEWORK_PATH")
                    cmake_path(APPEND v "DIR")
                    cmake_path(GET "v" PARENT_PATH v)
                    if(NOT "${v}" IN_LIST "cCppPropertiesConfigMacFrameworkPath")
                        list(APPEND cCppPropertiesConfigMacFrameworkPath "${v}")
                    endif()
                endforeach()
            endif()

            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_FORCED_INCLUDE}" STREQUAL "")
                set(cCppPropertiesConfigForcedInclude "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_FORCED_INCLUDE")
                    cmake_path(APPEND v "DIR")
                    cmake_path(GET "v" PARENT_PATH v)
                    if(NOT "${v}" IN_LIST "cCppPropertiesConfigForcedInclude")
                        list(APPEND cCppPropertiesConfigForcedInclude "${v}")
                    endif()
                endforeach()
            endif()

            if("${${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_BROWSE_PATH}" STREQUAL "")
                set(cCppPropertiesConfigBrowsePath "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_C_CPP_PROPERTIES_CONFIG_BROWSE_PATH")
                    cmake_path(APPEND v "DIR")
                    cmake_path(GET "v" PARENT_PATH v)
                    if(NOT "${v}" IN_LIST "cCppPropertiesConfigBrowsePath")
                        list(APPEND cCppPropertiesConfigBrowsePath "${v}")
                    endif()
                endforeach()
            endif()
            ...
            if("${listLength}" GREATER "0")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "compilerArgs" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "cCppPropertiesConfigCompilerArgs" "${i}" v)
                    string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "compilerArgs" "${i}" "\"${v}\"")
                endforeach()
            endif()
            if(NOT "${cCppPropertiesConfigIntelliSenseMode}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "intelliSenseMode" "\"${cCppPropertiesConfigIntelliSenseMode}\"")
            endif()
            list(LENGTH "cCppPropertiesConfigIncludePath" listLength)
            if("${listLength}" GREATER "0")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "includePath" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "cCppPropertiesConfigIncludePath" "${i}" v)
                    string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "includePath" "${i}" "\"${v}\"")
                endforeach()
            endif()
            list(LENGTH "cCppPropertiesConfigDefines" listLength)
            if("${listLength}" GREATER "0")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "defines" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "cCppPropertiesConfigDefines" "${i}" v)
                    string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "defines" "${i}" "\"${v}\"")
                endforeach()
            endif()
            if(NOT "${cCppPropertiesConfigCStandard}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "cStandard" "\"${cCppPropertiesConfigCStandard}\"")
            endif()
            if(NOT "${cCppPropertiesConfigCppStandard}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "cppStandard" "\"${cCppPropertiesConfigCppStandard}\"")
            endif()
            if(NOT "${cCppPropertiesConfigProvider}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "configurationProvider" "\"${cCppPropertiesConfigProvider}\"")
            endif()
            if(NOT "${cCppPropertiesConfigWindowsSdkVersion}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "windowsSdkVersion" "\"${cCppPropertiesConfigWindowsSdkVersion}\"")
            endif()
            list(LENGTH "cCppPropertiesConfigMacFrameworkPath" listLength)
            if("${listLength}" GREATER "0")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "macFrameworkPath" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "cCppPropertiesConfigMacFrameworkPath" "${i}" v)
                    string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "macFrameworkPath" "${i}" "\"${v}\"")
                endforeach()
            endif()
            list(LENGTH "cCppPropertiesConfigForcedInclude" listLength)
            if("${listLength}" GREATER "0")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "forcedInclude" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "cCppPropertiesConfigForcedInclude" "${i}" v)
                    string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "forcedInclude" "${i}" "\"${v}\"")
                endforeach()
            endif()
            if(NOT "${cCppPropertiesConfigCompileCommands}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "compileCommands" "\"${cCppPropertiesConfigCompileCommands}\"")
            endif()
            if(NOT "${cCppPropertiesConfigDotconfig}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "dotconfig" "\"${cCppPropertiesConfigDotconfig}\"")
            endif()
            if(NOT "${cCppPropertiesConfigMergeConfigs}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "mergeConfigurations" "${cCppPropertiesConfigMergeConfigs}")
            endif()
            string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "browse" "{}")
            if(NOT "${cCppPropertiesConfigBrowseLimitSymbolsToIncludedHeaders}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "browse" "limitSymbolsToIncludedHeaders" "${cCppPropertiesConfigBrowseLimitSymbolsToIncludedHeaders}")
            endif()
            list(LENGTH "cCppPropertiesConfigBrowsePath" listLength)
            if("${listLength}" GREATER "0")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "browse" "path" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "cCppPropertiesConfigBrowsePath" "${i}" v)
                    string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "browse" "path" "${i}" "\"${v}\"")
                endforeach()
            endif()
            if(NOT "${cCppPropertiesConfigBrowseDatabaseFile}" STREQUAL "")
                string(JSON cCppPropertiesConfig SET "${cCppPropertiesConfig}" "browse" "databaseFilename" "\"${cCppPropertiesConfigBrowseDatabaseFile}\"")
            endif()

            # Find index
            string(JSON cCppPropertiesConfigIndex LENGTH "${cCppProperties}" "configurations")
            if("${cCppPropertiesConfigIndex}" GREATER "0")
                math(EXPR listMaxIndex "${cCppPropertiesConfigIndex} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    string(JSON v GET "${cCppProperties}" "configurations" "${i}")
                    string(JSON vName GET "${v}" "name")
                    if("${vName}" STREQUAL "${cCppPropertiesConfigName}")
                        set(cCppPropertiesConfigIndex "${i}")
                        break()
                    endif()
                endforeach()
            endif()
            ...
            # Set item
            string(JSON cCppProperties SET "${cCppProperties}" "configurations" "${cCppPropertiesConfigIndex}" "${cCppPropertiesConfig}")
        endif()
    ...
    if(NOT "${${currentFunctionName}_LAUNCH_VAR}" STREQUAL "" OR NOT "${${currentFunctionName}_LAUNCH_FILE}" STREQUAL "")
        ...
        if(NOT "${${currentFunctionName}_LAUNCH_CONFIG_NAME}" STREQUAL "")
            ...
            if("${${currentFunctionName}_LAUNCH_CONFIG_PLUGINS}" STREQUAL "")
                set(launchConfigPlugins "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_LAUNCH_CONFIG_PLUGINS")
                    cmake_path(APPEND v "DIR")
                    cmake_path(GET "v" PARENT_PATH v)
                    if(NOT "${v}" IN_LIST "launchConfigPlugins")
                        list(APPEND launchConfigPlugins "${v}")
                    endif()
                endforeach()
            endif()

            if("${${currentFunctionName}_LAUNCH_CONFIG_SETUP_MACROS}" STREQUAL "")
                set(launchConfigSetupMacros "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_LAUNCH_CONFIG_SETUP_MACROS")
                    cmake_path(APPEND v "DIR")
                    cmake_path(GET "v" PARENT_PATH v)
                    if(NOT "${v}" IN_LIST "launchConfigSetupMacros")
                        list(APPEND launchConfigSetupMacros "${v}")
                    endif()
                endforeach()
            endif()

            if("${${currentFunctionName}_LAUNCH_CONFIG_SOURCE_FILE_MAP}" STREQUAL "")
                set(launchConfigSourceFileMap "")
            else()
                foreach(v IN LISTS "${currentFunctionName}_LAUNCH_CONFIG_SOURCE_FILE_MAP")
                    list(APPEND launchConfigSourceFileMap "${v}")
                endforeach()
            endif()
            ...
            list(LENGTH "launchConfigPlugins" listLength)
            if("${listLength}" GREATER "0")
                string(JSON launchConfig SET "${launchConfig}" "plugins" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "launchConfigPlugins" "${i}" v)
                    string(JSON launchConfig SET "${launchConfig}" "plugins" "${i}" "\"${v}\"")
                endforeach()
            endif()
            list(LENGTH "launchConfigSetupMacros" listLength)
            if("${listLength}" GREATER "0")
                string(JSON launchConfig SET "${launchConfig}" "setupMacros" "[]")
                math(EXPR listMaxIndex "${listLength} - 1")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "launchConfigSetupMacros" "${i}" v)
                    string(JSON launchConfig SET "${launchConfig}" "setupMacros" "${i}" "\"${v}\"")
                endforeach()
            endif()
            list(LENGTH "launchConfigSourceFileMap" listLength)
            if("${listLength}" GREATER "0")
                string(JSON launchConfig SET "${launchConfig}" "sourceFileMap" "{}")
                math(EXPR listMaxIndex "${listLength} - 1")
                string(TIMESTAMP quotePlaceHolder)
                set(quotePlaceHolder "<quote-${quotePlaceHolder}-quote/>")
                foreach(i RANGE "0" "${listMaxIndex}")
                    list(GET "launchConfigSourceFileMap" "${i}" v)
                    string(REPLACE "\\'" "${quotePlaceHolder}" v "${v}")
                    string(REPLACE "'" "\"" v "${v}")
                    string(REPLACE "${quotePlaceHolder}" "\\'" v "${v}")
                    string(JSON k MEMBER "{${v}}" "0")
                    string(JSON v GET "{${v}}" "${k}")
                    string(JSON launchConfig SET "${launchConfig}" "sourceFileMap" "${k}" "\"${v}\"")
                endforeach()
            endif()
            if(
