...
function(set_msvc_toolchain var)
    ...
    if("${tryCompileTargetType}" STREQUAL "STATIC_LIBRARY")
        string(APPEND result "" "\n")
        string(APPEND result "set(CMAKE_TRY_COMPILE_TARGET_TYPE \"${tryCompileTargetType}\"${cacheInstructions})" "\n")
    endif()
    string(APPEND result "" "\n")
...
function(set_gnu_toolchain var)
    ...
    string(APPEND result "" "\n")
    if("${os}" STREQUAL "Windows" OR "${compilerFileNameExt}" STREQUAL ".exe")
        if("${compilerFileNameNoExt}" STREQUAL "arm-none-eabi-gcc" OR "${compilerFileNameNoExt}" STREQUAL "arm-none-eabi-g++")
        ...
        if("${compilerFileNameNoExt}" STREQUAL "arm-none-eabi-gcc" OR "${compilerFileNameNoExt}" STREQUAL "arm-none-eabi-g++")
            string(APPEND result "set(CMAKE_C_COMPILER   \"${compilerDir}/arm-none-eabi-gcc\"${cacheInstructions})" "\n")
            string(APPEND result "set(CMAKE_CXX_COMPILER \"${compilerDir}/arm-none-eabi-g++\"${cacheInstructions})" "\n")
            string(APPEND result "set(CMAKE_ASM_COMPILER \"${compilerDir}/arm-none-eabi-gcc\"${cacheInstructions})" "\n")
            string(APPEND result "set(CMAKE_SIZE         \"${compilerDir}/arm-none-eabi-size\"${cacheInstructions})" "\n")
        else()
...
function(set_clang_toolchain var)
    ...
    get_filename_component(compilerFileNameExt "${path}" EXT)
    if("${compilerFileNameExt}" STREQUAL ".exe")
        set(exeSuffix ".exe")
    endif()
    ...
    if("${target}" STREQUAL "arm-none-eabi" AND NOT "${backPath}" STREQUAL "")
        if(NOT EXISTS "${backPath}")
            message(FATAL_ERROR "Not exists: '${backPath}'")
        endif()
...
function(set_iar_toolchain var)
    ...
    if("${os}" STREQUAL "Windows" OR "${compilerFileNameExt}" STREQUAL ".exe")
        ...
        message(FATAL_ERROR "[${CMAKE_CURRENT_FUNCTION}] Unsupported 'os': '${os}' 'compilerFileNameNoExt': '${compilerFileNameNoExt}' 'compilerFileNameExt': '${compilerFileNameExt}'")
    endif()
    if("${tryCompileTargetType}" STREQUAL "STATIC_LIBRARY")
        string(APPEND result "" "\n")
        string(APPEND result "set(CMAKE_TRY_COMPILE_TARGET_TYPE \"${tryCompileTargetType}\"${cacheInstructions})" "\n")
