...
find_program(TMP_CMAKE_AR "${CMAKE_AR}" NO_CACHE)
find_program(TMP_CMAKE_CXX_COMPILER "${CMAKE_CXX_COMPILER}" NO_CACHE)
find_program(TMP_CMAKE_C_COMPILER "${CMAKE_C_COMPILER}" NO_CACHE)
find_program(TMP_CMAKE_LINKER "${CMAKE_LINKER}" NO_CACHE)
find_program(TMP_CMAKE_MT "${CMAKE_MT}" NO_CACHE)
find_program(TMP_CMAKE_RC_COMPILER "${CMAKE_RC_COMPILER}" NO_CACHE)
find_program(TMP_CONAN_COMMAND "${CONAN_COMMAND}" NO_CACHE)
...
if(MSVC AND "20" STREQUAL "${CMAKE_CXX_STANDARD}" AND "${CMAKE_CXX_STANDARD_REQUIRED}")
    string(FIND "${CMAKE_CXX_FLAGS}" "-std=c++latest" "FLAGS_CONTAINS_STANDARD_1")
    string(FIND "${CMAKE_CXX_FLAGS}" "/std:c++latest" "FLAGS_CONTAINS_STANDARD_2")
    if("-1" STREQUAL "${FLAGS_CONTAINS_STANDARD_1}" AND "-1" STREQUAL "${FLAGS_CONTAINS_STANDARD_1}")
        if("" STREQUAL "${CMAKE_CXX_FLAGS}")
